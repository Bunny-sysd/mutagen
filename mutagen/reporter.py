...
def save_crash_report(crashes: list[dict], target_name: str, total_tested: int, patch_code: str = "", exploit_code: str = "", language: str = "c", binary_mode: bool = False, decompilation_info=None, profile: str = "legacy-audit", static_only: bool = False, raw_decompiled_code: str = "", clean_source_code: str = "", webhook_url: str = "", webhook_secret: str = "", webhook_headers: list[str] = None, sandboxed: bool = False, user_confirmed_unsandboxed: bool = False, docker_available: bool = False, extra_container_ids: list[str] = None, extra_container_images: list[str] = None, extra_container_digests: list[str] = None, reachability_status: str = "CONFIRMED", reachability_message: str = ""):
    ...
    # --- HTML REPORT ---------------------------------------------------
    html_file = f"crashes/report_{safe_name}_{timestamp}.html"
    ...
    mermaid_diagrams = [c.get("mermaid_sequence") for c in crashes if c.get("mermaid_sequence")]
    session_tab_btn = ""
    session_tab_html = ""
    if mermaid_diagrams:
        session_tab_btn = '<button class="tab-btn" onclick="showTab(\'session-tab\')">State Transitions</button>'
        session_tab_html = f"""
    <!-- STATE MACHINE VIEW -->
    <div id="session-tab" class="tab-content">
      <div class="table-container" style="padding: 2rem; text-align: center;">
        <h3 style="margin-bottom: 1.5rem; color: #00ff88; font-family: 'Outfit', sans-serif;">Stateful Protocol Progression Flow</h3>
        <pre class="mermaid" style="background: none; border: none;">
{mermaid_diagrams[0]}
        </pre>
      </div>
    </div>"""
...
<body>
  ...
  <!-- Prism.js & Mermaid.js for syntax highlighting & State Diagrams -->
